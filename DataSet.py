"""
生成数据集
你自己的数据可能不是 PyTorch 内置的数据集（比如 MNIST、CIFAR），而是你自己生成或收集的验证码图像。
为了让 PyTorch 可以像处理内置数据集一样处理你的数据，就需要自定义一个类(CaptchaDataset)来继承 Dataset：
"""
...
# 定义自己的数据集类CaptchaDataset继承Dataset，用于读取验证码数据
